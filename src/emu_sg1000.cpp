...
const uint32_t sg1000_palette_rgb[0x100] = {
...
static void gen_ntsc_pal_tables() {
    ...
    printf("uint32_t sg1000_4_phase[256] = {\n");
    int cc_width = 4;
    ...
    make_yuv_palette("_sg1000",rgb,256);
}
...
// big mem req
//uint8_t sg1000_videodata[256*240] = {0};   // larger than it needs to be at 60k

uint8_t* sg1000_videodata = 0;               // dynamically allocated
uint8_t* _sg1000_rom;

const char* _sg1000_ext[] = {
    "sg",
    0
};

const char* _sg1000_help[] = {
    "Keyboard:",
    "  Arrow Keys - D-Pad",
    "  Left Shift - Button 1",
    "  Option     - Button 2",
...
class EmuSG1000 : public Emu {
    uint8_t** _lines;
public:
    EmuSG1000(int ntsc) : Emu("sg1000",256,240,ntsc,(16 | (1 << 8)),4,EMU_SG1000)    // audio is 16bit
    {
        _lines = 0;
        _ext = _sg1000_ext;
        _help = _sg1000_help;
    ...
    virtual int info(const string& file, vector<string>& strs)
    {
        ...
        strs.push_back("");
        if (ext == "col")
            strs.push_back(::to_string((len + 0x3FF)/1024) + "k SG-1000");
        else
    ...
    void init_screen() {
        printf("init_screen\n");
        sg1000_videodata = new uint8_t[256*240];
        bitmap.data = sg1000_videodata + 24*256;
        ...
        const uint8_t* s = sg1000_videodata;
        for (int y = 0; y < 240; y++) {
            _lines[y] = (uint8_t*)s;
            s += 256;
        }
    ...
    {
        if (sg1000_videodata)
            memset(sg1000_videodata,0,256*240);
    }
    ...
    virtual int insert(const std::string& path, int flags, int disk_index)  {
		...
		printf("Inserting cartridge: [%s]\n",path.c_str());
            
		f = fopen(path.c_str(),"rb");
		if (!f) {
			printf("Error opening rom [%s]",path.c_str());
		} else {
			int size = fread(cart.rom,1,49152,f);
			printf("Read the cartridge: %d bytes.\n",size);
			fclose(f);
		}
...
/*		uint8_t buf[16];
        ...
        unmap_file(_sg1000_rom);
        
		_sg1000_rom = map_file(path.c_str(),len);
		
		cart.rom = _sg1000_rom; */

        emu_system_init(audio_frequency);
        
        // Not needed, is already being done in emu_system_init.
        // sg1000_init();
    ...
    const uint32_t _common_sg1000[16] = {
        0,  // msb
        0,
        0,
        INPUT_START<<8, // PLUS
    ...
    const uint32_t _classic_sg1000[16] = {
        INPUT_RIGHT,    // RIGHT
        INPUT_DOWN,     // DOWN
        0,              // LEFT_TOP
        INPUT_PAUSE<<8, // MINUS
    ...
    const uint32_t _generic_sg1000[16] = {
    ...
    virtual void hid(const uint8_t* d, int len)
    {
		...
		for (int i = 0; i < 2; i++) {
            ...
            if (ir) {
                int m = d[0] + (d[1] << 8);
                p = generic_map(m,_generic_sg1000);
                d += 2;
            } else
                p = wii_map(i,_common_sg1000,_classic_sg1000);
            input.pad[i] = p & 0xFF;
            if (i == 0)
                input.system = p >> 8;
    ...
    virtual const uint32_t* ntsc_palette() { return sg1000_4_phase; };
    virtual const uint32_t* pal_palette() { return _sg1000_4_phase_pal; };
    virtual const uint32_t* rgb_palette() { return sg1000_palette_rgb; };
...
Emu* NewSG1000(int ntsc)
{
    return new EmuSG1000(ntsc);
}
